...
import { Preference, Payment } from "mercadopago"; //estos elementos los traigo del SDK de mercadoPago
import Servicio from "../models/servicio.js";
import Pedido from "../models/pedido.js";

export const crearCarrito = async (req, res) => {
  try {
    const { serviciosCarrito } = req.body; // Ej: [{ id: '...', quantity: 2 }, { id: '...', quantity: 1 }]

    if (!serviciosCarrito || serviciosCarrito.length === 0) {
      return res.status(400).json({ mensaje: "El carrito está vacío" });
    }

    const idsServicios = serviciosCarrito.map((p) => p.id); //['665b9f1b1234567890abcdef','665ba0c49876543210fedcba',..]
    const serviciosDB = await Servicio.find({ _id: { $in: idsServicios } }); //$in buscar varios elementos a la vez cuando tenemos una lista de identificadores.

    const serviciosMap = new Map(serviciosDB.map((p) => [p._id.toString(), p])); //Un Map es una coleccion [clave,valor]
    // ['123adfh', {servicio: mantenimiento, precio: 100, descr}, {}]
    let totalPedido = 0;
    const itemsParaMP = [];
    const serviciosDelPedido = [];

    for (const itemCarrito of serviciosCarrito) {
      const servicioDB = serviciosMap.get(itemCarrito.id);
      if (servicioDB) {
        // Validar que la cantidad solicitada no exceda el stock (si tuvieras stock)
        itemsParaMP.push({
          title: servicioDB.nombreProducto,
          quantity: itemCarrito.quantity,
          currency_id: "ARS",
          unit_price: servicioDB.precio, // Usamos el precio de la BD, no del frontend
        });
        serviciosDelPedido.push({
          servicio: servicioDB._id,
          cantidad: itemCarrito.quantity,
        });
        totalPedido += servicioDB.precio * itemCarrito.quantity;
      }
    }
    ...
    if (itemsParaMP.length === 0) {
      return res
        .status(404)
        .json({
          mensaje: "Ninguno de los servicios del carrito fue encontrado.",
        });
    }

    const nuevoPedido = new Pedido({
      servicios: serviciosDelPedido,
      total: totalPedido,
  ...
  try {
    if (notification.type === "payment") {
      ...
      if (payment && payment.status === "approved") {
        ...
        if (!pedido) {
          console.error(
            `Pedido con ID ${pedidoId} no encontrado. Este webhook podría ser de un flujo de pago antiguo o inválido.`,
          );
        ...
        if (pedido.estado === "Pendiente") {
          ...
          console.log(
            `El pedido ${pedidoId} ya fue procesado. Estado actual: ${pedido.estado}`,
          );
        }
    ...
    console.error(
      `Error al procesar el webhook para el pago con ID: ${notification?.data?.id}`,
      error,
    );

    res
      .status(500)
      .json({ mensaje: "Error interno del servidor al procesar el webhook" });
  }
};
